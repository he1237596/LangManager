...
### 2026-04-14：会话安全 + 修改历史 + 日志管理

#### 会话安全

加入了空闲超时机制，符合互联网大厂的安全标准：

| 配置 | 时间 | 说明 |
|------|------|------|
| 空闲超时 | 2 天 | 无鼠标/键盘/滚动操作 |
| 提前提醒 | 超时前 30 分钟 | 弹窗提示，点击可继续操作 |
| 检查频率 | 每 60 秒 | 低频率，不影响性能 |

实现方式：在 `AuthContext` 中监听用户活动事件，通过 `setInterval` 定时检查空闲时长，到期后自动调用 `signOut()` 并跳转登录页。

```tsx
const IDLE_TIMEOUT_MS = 2 * 24 * 60 * 60 * 1000  // 2 天
const WARNING_BEFORE_MS = 30 * 60 * 1000          // 提前 30 分钟提醒
const CHECK_INTERVAL_MS = 60 * 1000                // 每 60 秒检查
```
...
DECLARE
  v_project_id UUID;
  v_log_enabled BOOLEAN;
BEGIN
  SELECT project_id INTO v_project_id
  FROM public.translation_keys WHERE id = NEW.key_id;
  SELECT log_enabled INTO v_log_enabled
  FROM public.projects WHERE id = v_project_id;
  IF v_log_enabled IS NOT TRUE THEN RETURN NEW; END IF;
  -- 记录变更历史...
END;
...
- **项目级开关**：`projects` 表新增 `log_enabled` 字段，每个项目可独立控制是否记录历史
- **仅超管可见**：历史按钮和日志管理面板仅超级管理员可访问
- **全语言查看**：点击操作列的历史图标，弹窗显示该 Key 所有语言的变更记录
- **修改人追踪**：每条记录显示修改人邮箱 + 修改时间

#### 日志管理

在项目设置页新增"日志管理" Tab（仅超管可见）：

- **开关**：一键启用/关闭翻译日志记录
- **统计**：显示当前项目的日志总条数
- **清理**：支持按时间范围清理（30 天前 / 90 天前 / 全部）
- **安全确认**：清理全部时需二次确认
...
```
translation_history      -- 翻译修改历史（触发器自动写入）
projects.log_enabled     -- 项目级日志开关
```

RLS 策略确保安全：超级管理员可删除任意项目历史，项目管理员只能删除自己项目的历史。
...
- **版本对比**：~~记录每次翻译修改，支持 diff 对比和回滚~~ ✅ 修改历史已实现（基于数据库触发器，含 diff 展示）
